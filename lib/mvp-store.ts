...
const metricKinds = ["count", "activity", "paidRuns"] as MetricKind[];
...
const seedItems: MvpItem[] = seedRecords.map((record, index) => ({
  ...
  descriptor: record[1],
  detail: record[2],
  priceUsdc: Number((0.2 + index * 0.01).toFixed(3)),
  usageCount: extractNumber(record[2], 12 + index * 7),
  paidRuns: extractNumber(record[1], 4 + index * 3),
  revenueUsdc: Number(((0.2 + index * 0.01) * (4 + index * 3)).toFixed(3)),
  status: "Active",
  payload: {
    label: record[0],
    descriptor: record[1],
...
function dbPath() {
  if (process.env.AGENTSWARM_DATA_FILE) {
    return process.env.AGENTSWARM_DATA_FILE;
  }
  return process.env.VERCEL
    ? path.join("/tmp", "agentswarm-db.json")
    : path.join(/*turbopackIgnore: true*/ process.cwd(), ".data", "agentswarm-db.json");
}
...
function slugify(value: string) {
  const base = value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return base || `agentswarm-item-${Date.now()}`;
}
...
export function recordPaidRun(
  ...
  const receipt: MvpReceipt = {
    id: `agentswarm-${itemSlug}-${Date.now()}`,
    itemSlug,
    itemName: item.name,
